...
// fork from https://github.com/andreasgal/node-wav
// @ts-nocheck
"use strict";
  ...
  while (pos < end) {
    ...
    switch (type) {
      ...
      case "data":
        ...
        let channelData = [];
        for (let ch = 0; ch < channels; ++ch)
          channelData[ch] = new Float32Array(
            new SharedArrayBuffer(samples * 4),
          );
...
function encode(channelData, opts) {
  ...
  let buffer = new SharedArrayBuffer(44 + samples * channels * (bitDepth >> 3));
